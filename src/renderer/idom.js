...
function resolveTagName (tname) {
  ...
  if (tname === 'slot' && !HTMLElement.prototype.attachShadow && HTMLElement.prototype.createShadowRoot) {
    return 'content';
  }
...
function adapter (tname, attrs, ...chren) {
...
function renderer ({ elem, render, shadowRoot }) {
...
const element = vdom.bind(null, adapter);
const h = (...args) => vdom.bind(null, adapter, ...args);

function builder (...tags) {
  return tags.length === 0 ? h : tags.map(tag => h.bind(null, tag));
}
